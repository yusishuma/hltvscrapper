...
exports.teams = async () => {
  try {
    ...
    teams.map((team) => {
      var url = 'https://www.hltv.org/team/' + team.hltvId + '/' + team.name.replace(/ /g, "-").toLowerCase();
      request.get(url).then((result) => {
        let overide$ = cheerio.load(result.res.text);
        let statusUrl = 'https://www.hltv.org/stats/teams/matches/' + team.hltvId + '/' + overide$('div.team-name.text-ellipsis').text();
